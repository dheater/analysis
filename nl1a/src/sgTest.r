...
# Generate test case data for testing the Savitzky-Galoy filter.
sgTest <- function (x) {

    #Create a dataframe that we will use to store the dataset.
    df=data.frame(x)

    # Create a vector listing the possible values of i.
    eyes = seq(2, 12)
    ...
    for(i in eyes) {
        ...
        sgy[,i-1] = savgol(x, filt_width, 2, 0)
        # Compute the first derivative.
        sgdy[,i-1] = savgol(x, filt_width, 2, 1)
    }

    # Give the generated matrices some more descriptive names.
    colnames(sgy) = paste("sgy", eyes, sep="")
    colnames(sgdy) = paste("sgdy", eyes, sep="")

    df=data.frame(x, sgy, sgdy)
}
